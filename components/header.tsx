...
export function Header() {
  ...
  return (
    <header className="w-full py-6 sm:py-8 px-4 sm:px-6 bg-cream-warm/90 backdrop-blur-sm sticky top-0 z-50 border-b border-border-subtle/50 safe-top mt-8">
      <nav className="container-responsive flex justify-between items-center">
        ...
        {isMobile && isOpen && (
          <div
            className={cn(
              "fixed inset-0 mb-6 bg-cream-warm paper-texture z-[100] transform transition-transform duration-300 ease-in-out",
              "flex flex-col safe-top safe-bottom",
              isOpen ? "translate-x-0" : "translate-x-full",
            )}
          >
            {/* Mobile menu header */}
            <div className="flex justify-between items-center p-6 sm:p-6  flex-shrink-0">
            ...
            <nav className="flex-1 flex flex-col items-center bg-cream-warm paper-texture justify-center py-8 px-4 space-y-6 sm:space-y-8">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
